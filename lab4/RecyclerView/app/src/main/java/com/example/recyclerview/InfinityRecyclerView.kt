...
class InfinityRecyclerView(inputStream: InputStream) : RecyclerView.Adapter<ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val currentPosition = position % database.size()
        val entry = database.getEntry(currentPosition)
        val type = entry.type
        val color = when (type) {
